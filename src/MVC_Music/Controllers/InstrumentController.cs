...
namespace MVC_Music.Controllers
{
    ...
    public class InstrumentController : ElephantController
    {
        ...
        public async Task<IActionResult> Index(int? page, int? pageSizeID)
        {
            ...
            ViewData["InstrumentCount"] = _context.Instruments.Count() + " Instruments in the System";
            //Handle Paging
        ...
        public async Task<IActionResult> InsertFromExcel(IFormFile theExcel)
        {
            ...
            if (theExcel != null)
            {
                ...
                if (fileLength > FileUploadValidator.MaxDocumentBytes)
                {
                    feedBack = $"Excel file cannot exceed {FileUploadValidator.MaxDocumentBytes / (1024 * 1024)} MB.";
                }
                else if (!(mimeType == "" || fileLength == 0))
                {
                        ...
                        || mimeType.Contains("spreadsheet", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (workSheet.Cells[1, 1].Text == "Instrument Name")
                        {
                            for (int row = start.Row + 1; row <= end.Row; row++)
                            {
                                ...
                                try
                                {
                                    // Get the Instrument Name row by row 
                                    instrument.Name = workSheet.Cells[row, 1].Text;
                                    //checking if that exists and then only saving just to be on safer side also checks in excel file before adding
                                    if (!_context.Instruments.Any(i => i.Name == instrument.Name))
                                    {
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            {
                //if the instrument is not created by the staff they can't edit that instrument
                if (instrument.CreatedBy != User.Identity!.Name)
                {
                    return Forbid(); //This prevents from requesting the edit action on instrument
                }
            }
