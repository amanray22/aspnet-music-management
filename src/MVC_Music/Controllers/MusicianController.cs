...
namespace MVC_Music.Controllers
{
    ...
    public class MusicianController : ElephantController
    {
            ...
            "Phone,DOB,SIN,InstrumentID")] Musician musician, string[] selectedOptions, List<IFormFile> theFiles, IFormFile? thePicture)
        {
            try
            {
                ...
                if (ModelState.IsValid
                    && await TryAddDocumentsAsync(musician, theFiles)
                    && await TryAddPictureAsync(musician, thePicture))
                {
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            //Check if the signed in role is staff or not 
            if (User.IsInRole("Staff"))
            {
                //if the musician is not created by the staff they can't edit that musician
                if (musician.CreatedBy != User.Identity!.Name)
                {
                    return Forbid(); //This prevents from requesting the edit action on Musician
                }
            }
        ...
        public async Task<IActionResult> Edit(int id, string[] selectedOptions, Byte[] RowVersion, string? chkRemoveImage, List<IFormFile> theFiles, IFormFile? thePicture)
        {
                ...
                p => p.SIN, p => p.InstrumentID))
            {
                try
                {
                    ...
                    else if (thePicture != null && !await TryAddPictureAsync(musicianToUpdate, thePicture))
                    {
                    ...
                    if (!await TryAddDocumentsAsync(musicianToUpdate, theFiles))
                    {
                        PopulateAssignedPlaysData(musicianToUpdate);
                        PopulateDropDownLists(musicianToUpdate);
                        return View(musicianToUpdate);
        ...
        public async Task<IActionResult> DeleteConfirmed(int id, Byte[] RowVersion)
        {
            ...
            try
            {
                ...
                await _context.SaveChangesAsync();
                return SafeRedirectToReturnUrl(ViewData["returnURL"]?.ToString());
            }
        ...
        private async Task<bool> TryAddDocumentsAsync(Musician musician, List<IFormFile>? theFiles)
        {
            if (theFiles == null)
            {
                return true;
            ...
            foreach (var f in theFiles.Where(f => f != null && f.Length > 0))
            {
                if (!FileUploadValidator.IsAllowedDocument(f, out var errorMessage))
                {
                    ModelState.AddModelError("theFiles", $"{Path.GetFileName(f.FileName)}: {errorMessage}");
                    return false;
                }
            }
            ...
            foreach (var f in theFiles.Where(f => f != null && f.Length > 0))
            {
                ...
                musician.MusicianDocuments.Add(new MusicianDocument
                {
                    MimeType = f.ContentType,
                    FileName = Path.GetFileName(f.FileName),
                    FileContent = new FileContent { Content = memoryStream.ToArray() }
                });
            }
        ...
        private async Task<bool> TryAddPictureAsync(Musician musician, IFormFile? thePicture)
        {
            if (thePicture == null)
            {
                return true;
            ...
            if (!FileUploadValidator.IsAllowedImage(thePicture, out var errorMessage))
            {
                ModelState.AddModelError("thePicture", errorMessage ?? "Invalid image file.");
                return false;
            }
            ...
            if (musician.MusicianPhoto != null)
            {
                ...
                musician.MusicianThumbnail = _context.MusicianThumbnails
                    .FirstOrDefault(p => p.MusicianID == musician.ID);
