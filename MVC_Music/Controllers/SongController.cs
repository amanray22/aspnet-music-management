...
namespace MVC_Music.Controllers
{
    ...
    public class SongController : ElephantController
    {
        ...
        public async Task<IActionResult> Delete(int? id)
        {
            ...
            //Check if the signed in role is supervisor or not 
            if (User.IsInRole("Supervisor"))
            {
                //if the song is not created by the Supervisor they can't edit that song
                if (song.CreatedBy != User.Identity!.Name)
                {
                    return Forbid(); //This prevents from requesting the edit action on Song
                }
            }
